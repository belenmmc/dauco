...
class ReaderService {
  Future<void> uploadData() async {
    try {
      var jsonData = await readExcel();
      await insertUsuariosFromJson(jsonData);
    } catch (e) {
      print("Error uploading data: $e");
    }
  }

  Future<Map<String, dynamic>> readExcel() async {
      ...
      for (var sheetName in excel.tables.keys) {
        ...
        for (var i = 1; i < sheet!.rows.length; i++) {
          ...
          for (var j = 0; j < headers!.length; j++) {
            rowData[headers[j]] = row.length > j ? row[j]?.value : null;
          }
  ...
  Future<void> insertUsuariosFromJson(Map<String, dynamic> jsonData) async {
    if (!jsonData.containsKey('Usuarios')) return;

    List<dynamic> usuarios = jsonData['Usuarios'];

    for (var usuario in usuarios) {
      if (usuario == null) continue;

      final id = usuario['responsable_id'] ?? 'NULL';
      final name = usuario['nombre'] ?? 'NULL';
      final surname = usuario['apellidos'] ?? 'NULL';
      final yesBool = usuario['si'] == true;
      final registeredAt = usuario['alta'] ?? 'NULL';
      final zone = usuario['zona'] ?? 'NULL';
      final minorsNum = usuario['num_menores'] ?? 'NULL';
      ...
      print(
          "Inserted into USUARIOS: $id, $name, $surname, $yesBool, $registeredAt, $zone, $minorsNum");
    }
  }
